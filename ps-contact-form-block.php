	...
	function __construct() {
		...
		if ( !$wpdb->get_results("SELECT * FROM $this->table_name WHERE name LIKE '%config%' LIMIT 1") ) {
			$this->config = (object) [
				...
				"redirectUrl" => get_home_url() . "/wp-admin/admin-post.php?action=api_auth",
				"token_type" => "Bearer",
				"expires_in" => 86399,
...
class UsefulCalendar {
	...
	function __construct($calendar) {
		...
		// all of these are in milliseconds; they assume the unit is minutes
		$this->slotBuffer = $calendar?->slotBuffer*60*1000;
		$this->slotDuration = $calendar?->slotDuration*60*1000;
		$this->slotInterval = $calendar?->slotInterval*60*1000;
		$this->slotPreBuffer = $calendar?->preBuffer*60*1000;
...
function ps_ghl_get_calendar_info( $location_id, $calendar_id = '' ) {
	...
	try {
		...
		$useful_calendar = new UsefulCalendar($calendars[$calendar_id]);
	...
	if ($_SERVER['REQUEST_METHOD'] === 'POST') {
		...
		try {	
			...
			ps_ghl_create_appointment(
				...
				$location_id,
				$updated_contact,
				substr($_POST['startTime'], 0, -3), // take off ms
				substr($_POST['endTime'], 0, -3)); // take off ms
...
function ps_handle_zip_request() {
	...
	$zips = array();
	$url = plugins_url('/zip-codes.csv', __FILE__);
