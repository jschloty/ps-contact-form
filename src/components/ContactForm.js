 ...
 */
function CurrentButtons({ pageState, loading, needsZip, existing }) {
  const [page, setPage] = pageState;
  return page == 1 && !existing ? (<>
      <Button id="pg1_button" type="submit" loading={loading}>Get a quote</Button>
      <p id="existing">Existing customer? <a href={document.location.origin + "/support"}>Click here</a> to contact us.</p>
    </>) 
    : page == 1 && existing ? (<>
      <Button id="pg1_button" type="submit" loading={loading}>Submit</Button>
    </>)
    : page == 2 ? (<>
      <p>No thanks.<br /><a href={document.location.origin}><SlArrowLeft /> Return to home</a></p>
...
function CurrentPage({ 
  ...
  //   let todayRounded = new Date((Math.floor(new Date().getTime()/(24*60*60*1000))*(24*60*60*1000))+(new Date().getTimezoneOffset()*60*1000));
  //   // does the same for the date in question
  //   let dateRounded = new Date((Math.floor(date.getTime()/(24*60*60*1000))*(24*60*60*1000))+(new Date().getTimezoneOffset()*60*1000));
  //   let isToday = todayRounded.getTime() == dateRounded.getTime();

  //   isDisabled = timeslots.length == 0 || (isToday && bookingAfter > closeTime);
  ...
  return (<ul id={"page" + page} className="page" page={page}>
    ...
    { (page == 1) ? <li key="compliance_disclaimer">
      <p id="sms-compliance" style={{ "white-space": "pre-line" }}>{compliance}</p>
    </li>: null}
    <CurrentButtons 
      pageState={[page, setPage]}
 ...
 * 
 * Does a few calculations to determine the starting position of the calendar.
 * @param {Object} calendarInfo A list of parameters about the calendar.
 * @returns The starting value of the calendar.
 */
function initCalendar( calendarInfo ) {
  const date = new Date();
  // sets open and close times based on today's date
  const openTime = new Date();
  openTime.setHours(calendarInfo.openHours[date.getDay()].openHour,
    calendarInfo.openHours[date.getDay()].openMinute,0,0);
  const closeTime = new Date();
  closeTime.setHours(calendarInfo.openHours[date.getDay()].closeHour,
    calendarInfo.openHours[date.getDay()].closeMinute,0,0);

  // uses UsefulCalendar's startTime calculation, rounded up to the nearest time slot
  const bookingAfter = new Date(Math.ceil((calendarInfo.startTime)
    /(calendarInfo.slotInterval))*(calendarInfo.slotInterval));

  let startTime;
  
  // if the calculated start time is during open hours
  if ( bookingAfter.getTime() > openTime.getTime() && bookingAfter.getTime() < closeTime.getTime() ) {
    startTime = bookingAfter;
  }
  // if start time is before open hours
  else if ( bookingAfter.getTime() < openTime.getTime() ) {
    startTime = openTime;
  } 
...
function ContactForm (props) {
  //default values for calendar params
  ...
  const { inputs, message, heading, content, nonce, existingCustomer, compliance } = props;
  ...
  let [appointmentTime, setAppointment] = useState(initialTime);
  ...
  async function checkValidity(e) {
    ...
    if (page == 3 && calendarInfo.current.id === "zip-input") {
      ...
      if (!validity.isValid) {
        ...
        //super fugly calendar initialization after zip is entered - debug use only though (hopefully)
        setAppointmentTime(initCalendar(calendarInfo.current));
        setGetLoading(false);
  ...
  if ((page == 2 || page == 3) && calendarInfo.current.id === "default") {
    ...
    .then((calendar) => {
      ...
      // initialTime = now.getHours() > 17 || new Date(now).setHours(0,0,0,0) < new Date(calendar.startTime).setHours(0,0,0,0) 
  ...
  return (
    <form noValidate={true} id="ps-contact-form" onSubmit={checkValidity}>
